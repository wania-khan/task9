    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {options.map((option) => (
          <div
            key={option.heading}
            className="bg-white rounded-lg shadow-md px-6 py-8 border border-12 border-gray-100 hover:bg-[#1D4ED8] hover:text-white">
            <h3 className="text-xl font-bold text-[#191D23] text-[22px] mb-2">
              {option.heading}
            </h3>
            <p className="text-[#64748B] text-base mb-4">{option.description}</p>
            <div className="flex items-center mb-4">
              <span className="mr-2 text-6xl font-semibold ">{option.price}</span>
              <span className="text-gray-700">{option.month}</span>
            </div>
            ...
            <div className="flex flex-wrap">
              {option.features_available && option.features_available.map((feature) => (
                ...
                <div className="w-8 h-8 rounded-full bg-[#E8EDFB] flex items-center justify-center">
                  ...
                  <span className="ml-2 text-[#191D23]">{feature}</span>
                </div>
              ))}
